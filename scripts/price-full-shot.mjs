...
const found = await page.evaluate(() => {
  ...
  const t = ps.find((p) => p.textContent?.trim() === "INITIAL FEE" || p.textContent?.trim() === "Initial Fee");
  if (!t) return null;
  let n = t.parentElement;
  while (n && !(n.tagName === "DIV" && n.style.borderTop)) n = n.parentElement;
  if (!n) return null;
  const r = n.getBoundingClientRect();
  return { top: r.top + window.scrollY, height: Math.ceil(r.height) };
});
if (!found) {
  console.warn("INITIAL FEE not found");
  process.exit(1);
}
const pad = 30;
await page.evaluate(
...
await page.screenshot({
  path: path.resolve("screenshots/redesign/04-price-full.png"),
  clip: { x: 0, y: 0, width: 1440, height: captureH },
});
console.log("Saved price-full h=" + captureH);
await ctx.close();
await browser.close();
